...
#' PredictedData <- PredictionCaller(NormalizedData=NormalizedData,model=FMDV_model)
#'
#' QualityControl(PredictedData,model=FMDV_model)
#'
#'
QualityControl<-function(data,
  ...
  if ("FMDV serotypes nomenclature"==model$info){
    data$Length_QC<-(data$Length>(1-Length_value)*8200)&(data$Length<(1+Length_value)*8200)
  }
...
#' PredictedData <- PredictionCaller(NormalizedData=NormalizedData,model=FMDV_model)
#'
#' PredictedData <- QualityControl(PredictedData,model=FMDV_model)
#'
